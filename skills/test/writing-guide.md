...
A full user flow spanning multiple pages (checkout, sign-up) gets one end-to-end test, not a unit test per page it touches.

## Regression tests from `patch`

Write it first in a failing state against the pre-fix code if that's still checked out, then confirm it passes after the fix. That sequence — fail, then pass — is what proves the fix actually fixes it, not just that a new test happens to pass.
