...
Signals: folders or modules named after business concepts, not technical layers (`orders/`, `billing/`, `catalog/`), each bundling its own models, logic, and routes together; a shared kernel or `common/` folder for concerns that cut across domains; naming that mirrors terms the product actually uses (a `Cart`, not a `BasketEntity`).
