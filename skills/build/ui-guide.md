...
Treat these differently. A **screen** (a page, a route) owns its own layout and every state it can be in. A **piece** (a reusable component) just takes inputs and renders — it doesn't own page-level layout or navigation.

## Placeholder images

If a screen needs images it doesn't have real assets for, ask which of three: the developer will add real files later, a stand-in image service wired at the right sizes, or plain colored placeholders. Never invent a path to a file that doesn't exist.
