...
Backend and full-stack work (an API endpoint, a data-backed page, a service) builds in this order. A pure front-end piece against an already-built API skips straight to the UI half.
...
3. **Endpoints or routes**, built exactly to the spec's API contract. Check not just that a caller is authenticated but that they're allowed to touch this specific resource. Rate-limit anything public-facing.
