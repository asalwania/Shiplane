...
5. Write acceptance criteria as checkable conditions, each naming where its value comes from (an API field, a token, a fixed number), so a gap is visible now instead of surfacing mid build.
