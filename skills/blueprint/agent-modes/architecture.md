...
- **Framework and rendering default** — recommend one (with a runner-up) based on the flows `outline` recorded: content-heavy and SEO-sensitive leans SSR/SSG, interaction-heavy leans CSR with selective SSR.
...
2. If a decision could be helped by an outside connector or a ready-made skill, ask first (`internal/tool-discovery.md`) before searching.
