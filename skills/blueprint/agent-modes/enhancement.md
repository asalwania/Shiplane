...
Read this when a decision that already has a spec (`Proposed`, `Assumed`, or `Accepted`) is genuinely being overturned, not extended. A new requirement invalidates the rendering choice; the token scale is being replaced; an `Assumed` spec's guess turned out wrong.
...
4. Check every other spec and scope item that referenced the superseded one; flag each to be read again rather than assuming the reversal only touches the one you started from.
...
Point to `build` for any work already built that now needs to change to match the new spec; flag it clearly as rework, not new work.
