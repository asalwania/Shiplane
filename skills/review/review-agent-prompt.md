# Spawn template: cross-model review subagent
...
Map to a reviewer that differs from the author. If no differing model is available in this environment, review inline on the author's model and say so plainly in the report — a degraded review, not the cross-model guarantee. Never silently accept a same-model review as if it were independent.
...
If the subagent errored or wrote nothing, say so and offer to re-run — never relay a fabricated or empty review. Otherwise relay its findings per [review-guide.md](review-guide.md)'s report format.
