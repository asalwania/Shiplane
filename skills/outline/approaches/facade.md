...
Recorded in the scope file; `build` reads it to know a screen built under this approach is expected to stand up on fake data now, with the real data layer wired in as a separate, later item, not a shortcut `build` invents on its own partway through.
