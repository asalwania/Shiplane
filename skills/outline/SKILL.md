...
2. **Greenfield** (no code yet): ask what the product is, who uses it, and the two or three flows that matter most. Don't ask about framework or hosting — that's `blueprint`'s call once the idea is clear.
...
5. **Record the browser/device support target** (e.g. "last 2 versions of evergreen browsers, no IE, mobile-first down to 360px"). Every later `blueprint` and `trial` pass reads this instead of guessing.
...
Point to `blueprint` for the first item that has an open load-bearing decision, or straight to `build` if the decision already exists.
