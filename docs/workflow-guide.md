...
**outline** (`plan-greenfield` mode). The idea is an online storefront. `outline` asks what it sells and who buys it, then breaks the idea into an ordered list: listing page, detail page, cart drawer, checkout. It records the support target ("evergreen browsers, mobile-first from 360px"), picks the `skateboard` delivery approach (ship the smallest usable storefront, then grow it, rather than perfecting one journey before the next), and picks `Beta` as the workflow depth — real users, but pre-launch, so `test` matters more than a fresh-model `review` does yet.

**blueprint** (`architecture` mode). No stack exists yet, so this runs before any page-level decision. It recommends Next.js (SSR fits a catalog that needs to be indexable), Tailwind, and a Postgres-backed API, each with a one-line reason, and writes the pick as `docs/specs/000-stack.md`.

**survey** (`greenfield` mode). Runs right after the project is scaffolded with that stack. `survey` reads `package.json` and the config files, samples a couple of real routes, and writes `AGENTS.md`: framework, routing convention (`app/` directory), styling system (Tailwind, tokens in `tailwind.config.ts`), the real `npm run dev`/`build`/`test` commands, and — sampling a couple of API route files — notes the project leans domain-driven (an `orders/` module bundling its own routes and logic), so it records that convention too.

**blueprint** (`feature` mode). The listing page is next in `outline`'s build order. Two decisions are open: how the page is rendered, and the page-size/response shape for `/api/products`. `blueprint` decides SSR (product data changes often, and it needs to be indexable), defines the API contract (`GET /api/products?page=`, response shape, 24 items per page), points at the existing token scale for cards and spacing, and sets a performance budget (largest contentful paint under 2.5s on a mid-tier mobile device). It writes `docs/specs/003-product-listing.md` with acceptance criteria: "page 2 of results loads via the documented endpoint," "each card exposes name, price, and image with real alt text," "layout holds at 360px and 1440px."
